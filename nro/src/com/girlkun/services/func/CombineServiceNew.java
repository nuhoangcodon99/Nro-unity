...
public class CombineServiceNew {
    ...
    public static final int DOI_DIEM_VIP = 5959;
    ...
    public static final int DAP_SET_KICH_HOAT_TS_VIP = 10101;
    ...
    public static final int AN_TRANG_BI_VIP = 51627;
    public static final int SAO_9 = 57627;
            ...
            case THUC_TINH_DT:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (ngocrongsp != null && ngocrongsp.quantity >= 3 && dathuctinh != null && dathuctinh.quantity >= 30
                        ...
                        if (player.nPoint.hpg >= 600000 && player.nPoint.mpg >= 600000 && player.nPoint.dameg >= 28000) {
                            this.lmht.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                    "Bạn đã đạt cấp độ thức tỉnh tối đa tối đa", "Đóng");
                            return;
                        }
                        if (player.nPoint.dameg >= 28000 && player.nPoint.hpg >= 600000
                                && player.nPoint.mpg >= 600000 && player.nPoint.power >= 90000000000L) {
                            String npcSay = "|7|[ THỨC TỈNH BẢN THÂN ]"
                                    + "\n|5|HP: " + player.nPoint.hpg
                                    + "\nKI: " + player.nPoint.mpg
                                    + "\nSD: " + player.nPoint.dameg
                                    + "\n|7|Tỉ lệ thành công: "
                                    + (player.nPoint.hpg >= 600000 && player.nPoint.hpg < 800000
                                    && player.nPoint.mpg >= 600000 && player.nPoint.mpg < 800000
                                    && player.nPoint.dameg >= 28000 && player.nPoint.dameg < 30000 ? "50%"
                                            : player.nPoint.hpg >= 600000 && player.nPoint.hpg < 800000
                                            && player.nPoint.mpg >= 600000 && player.nPoint.mpg < 800000
                                            && player.nPoint.dameg >= 28000 && player.nPoint.dameg < 30000 ? "30%" : "10%") + "\n";
                        ...
                        if (player.pet.nPoint.dameg >= 28000 && player.pet.nPoint.hpg >= 600000
                                && player.pet.nPoint.mpg >= 600000 && player.pet.nPoint.power >= 90000000000L) {
                            String tsSay = "|7|[ THỨC TỈNH ĐỆ TỬ ]\n" + player.pet.name.substring(1)
                                    + "\n|5|Cấp bậc : " + player.pet.getNameThuctinh(player.pet.thuctinh)
                                    + "\nHP: " + (player.pet.nPoint.hpg >= 600000 ? Util.format(player.pet.nPoint.hpg)
                                            : Util.format(player.pet.nPoint.hpg) + " Cấp Sau: " + Util.format(player.pet.nPoint.hpg + 500))
                                    + "\nKI: " + (player.pet.nPoint.mpg >= 600000 ? Util.format(player.pet.nPoint.mpg)
                                            : Util.format(player.pet.nPoint.mpg) + " Cấp Sau: " + Util.format(player.pet.nPoint.mpg + 500))
                                    + "\nSD: " + (player.pet.nPoint.dameg >= 28000 ? Util.format(player.pet.nPoint.dameg)
                                            : Util.format(player.pet.nPoint.dameg) + " Cấp Sau: " + Util.format(player.pet.nPoint.dameg + 200))
                                    + "\n|7|Tỉ lệ thành công: "
                                    + (player.pet.nPoint.hpg >= 600000 && player.pet.nPoint.hpg < 620000
                                    && player.pet.nPoint.mpg >= 600000 && player.pet.nPoint.mpg < 620000
                                    && player.pet.nPoint.dameg >= 28000 && player.pet.nPoint.dameg < 34000 ? "50%"
                                            : player.pet.nPoint.hpg >= 60000 && player.pet.nPoint.hpg < 640000
                                            && player.pet.nPoint.mpg >= 600000 && player.pet.nPoint.mpg < 640000
                                            && player.pet.nPoint.dameg >= 28000 && player.pet.nPoint.dameg < 36000 ? "30%" : "10%") + "\n";
...
// -----------------------------------------------------------------------------                  
            ...
            case DAP_SET_KICH_HOAT_TS:
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (dhd.isSKH() && dhd != null && dtl != null) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta cần Trang Bị Thần Linh chưa có SKH và 500 viên Đá Ngũ Sắc", "Đóng");
                    } else if (dhd != null && dtl != null) {
                        String npcSay = "|6|" + dhd.template.name + "\n";
                        for (Item.ItemOption io : dhd.itemOptions) {
                            npcSay += "|2|" + io.getOptionString() + "\n";
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta cần Trang Bị Thiên Sứ và 500 viên Đá Ngũ Sắc", "Đóng");
                    }
                }
                break;
             case DAP_SET_KICH_HOAT_TS_VIP:
                if (player.combineNew.itemsCombine.size() == 2) {
                    Item dhd = null, dtl = null;
                    for (Item item : player.combineNew.itemsCombine) {
                        if (item.isNotNullItem()) {
                    ...
                    if (dhd.isSKH() && dhd != null && dtl != null) {
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta cần Trang Bị Thiên sứ chưa có SKH và 999 viên ngọc 1 sao", "Đóng");
                    } else if (dhd != null && dtl != null) {
                        String npcSay = "|6|" + dhd.template.name + "\n";
                        for (Item.ItemOption io : dhd.itemOptions) {
                            npcSay += "|2|" + io.getOptionString() + "\n";
                        }
                        npcSay += "Ngươi có muốn nâng cấp\n";
                        npcSay += "|1|" + getNameItemC1(dhd.template.gender, dhd.template.type)
                                + " (ngẫu nhiên Set Kích Hoạt)\n|7|Tỉ lệ thành công " + (dtl != null ? "100%" : "40%") + "\n|2|Cần "
                                + Util.numberToMoney(COST_DAP_DO_KICH_HOAT) + " Vàng" + Util.numberToMoney(RUBY_DAP_DO_KICH_HOAT) + " Hồng Ngọc" + Util.numberToMoney(GEM_DAP_DO_KICH_HOAT) + " Ngọc Xanh";
                        if (player.inventory.gold >= COST_DAP_DO_KICH_HOAT) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta cần Trang Bị Thiên Sứ và 999 viên ngọc 1 sao", "Đóng");
                    }
                }
                break;
            ...
            case AN_TRANG_BI:
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                    if (player.combineNew.itemsCombine.size() == 2) {
                        ...
                        if (isTrangBiAn(item)) {
                            if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem() && dangusac.quantity >= 99) {
                                String npcSay = item.template.name + "\n|2|";
                                for (Item.ItemOption io : item.itemOptions) {
                                    npcSay += io.getOptionString() + "\n";
                                }
                    ...
                    if (player.combineNew.itemsCombine.size() == 2) {
                        ...
                        if (isTrangBiAn(item)) {
                            if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem() && dangusac.quantity >= 99) {
                                ...
                                npcSay += "|1|Con có muốn biến trang bị " + item.template.name + " thành\n"
                                        + "trang bị vip không?\b|4|Tỉ lệ thành công: 10%\n"
                                        + "|7|Cần 99 " + dangusac.template.name + "\nCần " + Util.numberToMoney(COST) + " vàng" + "\nCần " + Util.numberToMoney(RUBY) + "hồng ngọc";
                                this.baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE, npcSay, "Làm phép", "Từ chối");
                            } else {
                                this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn chưa bỏ đủ vật phẩm !!!", "Đóng");
            ...
            case SAO_9:
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
                    if (player.combineNew.itemsCombine.size() == 2) {
                        ...
                        Item dangusac = player.combineNew.itemsCombine.get(1);
                        if (isTrangBiAn(item)) {
                            if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem() && dangusac.quantity >= 10) {
                                String npcSay = item.template.name + "\n|2|";
                                for (Item.ItemOption io : item.itemOptions) {
                                    npcSay += io.getOptionString() + "\n";
                                }
                                npcSay += "|1|Con có muốn cường hóa " + item.template.name + " thành\n"
                                        + "trang bị có 9 spl không?\b|4|Tỉ lệ thành công: 5%\n"
                                        + "|7|Cần 10 " + dangusac.template.name + "\nCần " + Util.numberToMoney(COST) + " vàng" + "\nCần " + Util.numberToMoney(RUBY) + "hồng ngọc";
                                this.baHatMit.createOtherMenu(player, ConstNpc.MENU_START_COMBINE, npcSay, "Làm phép", "Từ chối");
                            } else {
                                this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Bạn chưa bỏ đủ vật phẩm !!!", "Đóng");
                            }
            ...
            case DOI_DIEM_VIP:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.npsthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Con hãy đưa cho ta Phiếu Boss", "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 1) {
                    List<Integer> itemdov2 = new ArrayList<>(Arrays.asList(1576, 1577));
                    int couponAdd = 0;
                    Item item = player.combineNew.itemsCombine.get(0);
                    if (item.isNotNullItem()) {
                        if (item.template.id >= 1576 && item.template.id <= 1577) {
                            couponAdd = itemdov2.stream().anyMatch(t -> t == item.template.id) ? 1 : item.template.id <= 1577 ? 1 : 1;
                        }
                    }
                    if (couponAdd == 0) {
                        this.npsthiensu64.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Phiếu Boss", "Đóng");
                        return;
                    }
                    String npcSay = "|2|Sau khi nâng cấp được 100₫\n|7|"
                            + "Bạn sẽ nhận được : " + couponAdd + " điểm\n"
                            + (500000000 > player.inventory.gold ? "|7|" : "|100|")
                            + "Cần " + Util.numberToMoney(500000000) + " vàng";
            ...
            case NANG_CAP_BONG_TAI_CAP4:
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (bongTai != null && mvbt != null && mvbt.quantity >= 9999) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần 1 Bông tai Porata cấp 3 , 10K Hồng Ngọc, 500 Triệu Vàng , X9999 Mảnh vỡ bông tai "
                                + " và X9999 Mảnh hồn bông tai", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần 1 Bông tai Porata cấp 2 , 10K Hồng Ngọc, 500 Triệu Vàng , X999 Mảnh vỡ bông tai "
            ...
            case MO_CHI_SO_BONG_TAI_CAP4:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (bongTai != null && thachPhu != null && daXanhLam != null && thachPhu.quantity >= 999) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần 1 Bông tai Porata cấp 4, X999 Mảnh hồn bông tai và 5 Đá xanh lam", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần 1 Bông tai Porata cấp 4, X999 Mảnh hồn bông tai và 5 Đá xanh lam", "Đóng");
            ...
            case CHE_TAO_TRANG_BI_TS:
                ...
                if (player.combineNew.itemsCombine.size() == 1 && player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaNangCap()).count() != 1 || player.combineNew.itemsCombine.size() == 4 && player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaNangCap()).count() != 1) {
                    Service.getInstance().sendThongBao(player, "Thiếu Đá Nâng Cấp");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 1 && player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaMayMan()).count() != 1 || player.combineNew.itemsCombine.size() == 4 && player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isDaMayMan()).count() != 1) {
                    ...
                    if (Util.nextInt(0, 100) < tilemacdinh) {
                        ...
                        if (tilemacdinh > 0) {
                            for (byte i = 0; i < itemTS.itemOptions.size(); i++) {
                                if (itemTS.itemOptions.get(i).optionTemplate.id != 0 && itemTS.itemOptions.get(i).optionTemplate.id != 20) {
    ...
    private void doidiemvip(Player player) {
        if (player.combineNew.itemsCombine.size() == 1) {
            player.inventory.gold -= 0;
            List<Integer> itemdov2 = new ArrayList<>(Arrays.asList(1576, 1577));
            Item item = player.combineNew.itemsCombine.get(0);
            ...
            if (item.quantity < 100) {
                Service.gI().sendThongBaoOK(player, "không đủ phiếu boss");
            } else if (item.quantity >= 100) {
                InventoryServiceNew.gI().sendItemBags(player);
                player.inventory.coupon += 100;
                Service.gI().sendThongBaoOK(player, "Bú 100 Điểm");
...
//-------------------------------------------------------------------------------      
        ...
        if (player.combineNew.itemsCombine.size() == 2) {
            ...
            if (dhd != null) {
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0 && player.inventory.gold >= COST_DAP_DO_KICH_HOAT && player.inventory.ruby >= RUBY_DAP_DO_KICH_HOAT && player.inventory.gem >= GEM_DAP_DO_KICH_HOAT) {
                    ...
                    InventoryServiceNew.gI().subQuantityItemsBag(player, dhd, 1);
                    if (dtl != null) {
                        InventoryServiceNew.gI().subQuantityItemsBag(player, dtl, 500);
                    }
                    InventoryServiceNew.gI().sendItemBags(player);
                    Service.gI().sendMoney(player);
                    reOpenItemCombine(player);
    ...
    private void dapDoKichHoatTSVip(Player player) {
        if (player.combineNew.itemsCombine.size() == 2) {
            Item dhd = null, dtl = null;
            for (Item item : player.combineNew.itemsCombine) {
                if (item.isNotNullItem()) {
            ...
            if (dhd != null) {
                if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0 && player.inventory.gold >= COST_DAP_DO_KICH_HOAT && player.inventory.ruby >= RUBY_DAP_DO_KICH_HOAT && player.inventory.gem >= GEM_DAP_DO_KICH_HOAT) {
                    ...
                    InventoryServiceNew.gI().subQuantityItemsBag(player, dhd, 1);
                    if (dtl != null) {
                        InventoryServiceNew.gI().subQuantityItemsBag(player, dtl, 999);
                    }
                    InventoryServiceNew.gI().sendItemBags(player);
                    Service.gI().sendMoney(player);
                    reOpenItemCombine(player);
...
//            }           
        ...
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (Util.isTrue(50, 100)) {
                ...
                if (player.inventory.ruby < 100000) {
                    ...
                    if (optionLevel_216 == null && level_102 == 8 && Trangbi != null && DaBV != null && LuaThan != null) {
                        if (DaBV.quantity < 100 || LuaThan.quantity < 100) {
                            ...
                            if (Util.isTrue(40, 100)) {
                                ...
                                Trangbi.itemOptions.add(new ItemOption(72, 16));
    ...
    private void psHoaTrangBi(Player player) {
        ...
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.isTrangBiHacHoa()).count() != 1) {
            Service.getInstance().sendThongBao(player, "Thiếu trang bị bí ẩn");
            return;
        }
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1390).count() != 1) {
            Service.getInstance().sendThongBao(player, "Thiếu đá bí ẩn");
            return;
        }
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            if (player.inventory.gold < COST) {
                Service.getInstance().sendThongBao(player, "Con cần thêm vàng để Bí ẩn hóa...");
                return;
            }
            ...
            Item daHacHoa = player.combineNew.itemsCombine.stream().filter(item -> item.template.id == 1390).findFirst().get();
            ...
            if (Util.isTrue(50, 100)) {
                ...
                Service.getInstance().sendThongBao(player, "Bạn đã bí ẩn hóa thành công");
            } else {
                sendEffectFailCombine(player);
            }
...
//                                        sendEffectFailCombine(player);
    ...
    private void tayHacHoaTrangBi(Player player) {
        ...
        if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.id == 1391).count() != 1) {
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            ...
            player.inventory.gold -= 0;
            Item buagiaihachoa = player.combineNew.itemsCombine.stream().filter(item -> item.template.id == 1391).findFirst().get();
            Item trangBiHacHoa = player.combineNew.itemsCombine.stream().filter(Item::isTrangBiHacHoa).findFirst().get();
            ...
            if (!player.combineNew.itemsCombine.isEmpty()) {
                ...
                if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()&& dangusac.quantity >= 99) {
                    if (optionStar == null) {
                        ...
                        } else if (dangusac.template.id == 15) {
                            if (Util.isTrue(40, 100)) {
                        ...
                        } else if (dangusac.template.id == 16) {
                            if (Util.isTrue(40, 100)) {
...
//                    sendEffectCombineDB(player, item.template.iconID);
    ...
    private void antrangbivip(Player player) {
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {

            if (player.inventory.gold < COST) {
                Service.getInstance().sendThongBao(player, "Con cần thêm vàng để nâng cấp...");
                return;
            }
            if (player.inventory.ruby < RUBY) {
                Service.getInstance().sendThongBao(player, "Con cần thêm hồng ngọc để nâng cấp...");
                return;
            }
            ...
            if (!player.combineNew.itemsCombine.isEmpty()) {
                ...
                Item.ItemOption optionStar = null;
                for (Item.ItemOption io : item.itemOptions) {
                    if (io.optionTemplate.id == 117 || io.optionTemplate.id == 218) {
                        star = io.param;
                        optionStar = io;
                        break;
                    }
                ...
                if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()&& dangusac.quantity >= 99) {
                    if (optionStar == null) {
                        ...
                        if (dangusac.template.id == 1570) {
                            if (Util.isTrue(10, 200)) {
                                item.itemOptions.add(new Item.ItemOption(218, 10));
                                item.itemOptions.add(new Item.ItemOption(117, 5));
                                item.itemOptions.add(new Item.ItemOption(72, 16));
                                sendEffectSuccessCombine(player);
                            } else {
                                sendEffectFailCombine(player);
                            }
...
//                    sendEffectCombineDB(player, item.template.iconID);
                        ...
                        Service.getInstance().sendThongBao(player, "Trang bị của bạn có vip rồi mà !!!");
                    }
                }
            }
        }
    }
    private void sao9(Player player) {
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {
            ...
            if (!player.combineNew.itemsCombine.isEmpty()) {
                ...
                Item.ItemOption optionStar = null;
                for (Item.ItemOption io : item.itemOptions) {
                    if (io.optionTemplate.id == 238 || io.optionTemplate.id == 239) {
                        star = io.param;
                        optionStar = io;
                        break;
                    }
                ...
                if (item != null && item.isNotNullItem() && dangusac != null && dangusac.isNotNullItem()&& dangusac.quantity >= 99) {
                    if (optionStar == null) {
                        ...
                        if (dangusac.template.id == 1579) {
                            if (Util.isTrue(10, 200)) {
                                item.itemOptions.add(new Item.ItemOption(107, 9));
                                item.itemOptions.add(new Item.ItemOption(238, 0));
                                item.itemOptions.add(new Item.ItemOption(239, 0));
                                sendEffectSuccessCombine(player);
                            } else {
                                sendEffectFailCombine(player);
                            }
...
//                    sendEffectCombineDB(player, item.template.iconID);
    ...
    private void moChiSoBongTaicap4(Player player) {
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (bongtai != null && honbongtai != null && honbongtai.quantity >= 999) {
                player.inventory.gold -= gold;
                player.inventory.gem -= gem;
                InventoryServiceNew.gI().subQuantityItemsBag(player, honbongtai, 999);
                ...
                if (Util.isTrue(player.combineNew.ratioCombine, 100)) {
                    ...
                    int rdUp = Util.nextInt(0, 7);
                    if (rdUp == 0) {
                        bongtai.itemOptions.add(new Item.ItemOption(50, Util.nextInt(5, 50)));
                    } else if (rdUp == 1) {
                        bongtai.itemOptions.add(new Item.ItemOption(77, Util.nextInt(5, 50)));
                    } else if (rdUp == 2) {
                        bongtai.itemOptions.add(new Item.ItemOption(103, Util.nextInt(5, 50)));
                    } else if (rdUp == 3) {
                        bongtai.itemOptions.add(new Item.ItemOption(108, Util.nextInt(5, 50)));
                    } else if (rdUp == 4) {
                        bongtai.itemOptions.add(new Item.ItemOption(94, Util.nextInt(5, 50)));
                    } else if (rdUp == 5) {
                        bongtai.itemOptions.add(new Item.ItemOption(14, Util.nextInt(5, 50)));
                    } else if (rdUp == 6) {
                        bongtai.itemOptions.add(new Item.ItemOption(80, Util.nextInt(5, 50)));
                    } else if (rdUp == 7) {
                        bongtai.itemOptions.add(new Item.ItemOption(81, Util.nextInt(5, 50)));
                    }
                    sendEffectSuccessCombine(player);
                } else {
                    sendEffectFailCombine(player);
...
//                
    ...
    private String getTextTopTabCombine(int type) {
        switch (type) {
            ...
            case AN_TRANG_BI_VIP:
                return "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở thành trang bị cấp 16";
            case SAO_9:
                return "Ta sẽ phù phép\ncho trang bị của ngươi\ntrở thành trang bị ngon";
            case NHAP_NGOC_RONG:
            ...
            case DAP_SET_KICH_HOAT_TS:
                return "Đưa ta 500 Đá Ngũ Sắc và 1 Trang Bị Thần Linh \n Ta sẽ phù phép thành Set Kích Hoạt!";
            case DAP_SET_KICH_HOAT_TS_VIP:
                return "Đưa ta 999 viên ngọc 1 sao và 1 Trang Bị Thiên sứ \n Ta sẽ phù phép thành Set Kích Hoạt thiên sứ!";
            case TAY_SKH_TRANG_BI:
    ...
    private String getTextInfoTabCombine(int type) {
        switch (type) {
            ...
            case NANG_CAP_BONG_TAI_CAP3:
                return "Vào hành trang\nChọn Porata Cấp 3 và 999 mảnh vỡ bông tai\nSau đó chọn 'Nâng cấp'";
            case NANG_CAP_BONG_TAI_CAP4:
                return "Vào hành trang\nchọn bông tai Porata cấp 3\nchọn x9999 MVBT C4\nSau đó chọn 'Nâng cấp' ";
            case MO_CHI_SO_BONG_TAI:
                return "Vào hành trang\nChọn Porata Cấp 2 + 99 mảnh hồn bông tai + 1 Đá xanh lam \nSau đó chọn 'Nâng cấp'";
            case MO_CHI_SO_BONG_TAI_CAP4:
                return "Vào hành trang\nChọn Porata Cấp 4 + 999 mảnh hồn bông tai + 5 Đá xanh lam \nSau đó chọn 'Nâng cấp'";
            case MO_CHI_SO_BI_TICH:
            ...
            case DAP_SET_KICH_HOAT_HD:
                return "Cần 1 Trang Bị Thần Linh bất kỳ và 150 Đá Ngũ Sắc"
                        + "Sau đó chọn 'Nâng cấp'";
            case DAP_SET_KICH_HOAT_TS:
                return "Cần 1 Trang Bị Thần Linh bất kỳ và 500 Đá Ngũ Sắc"
                        + "Sau đó chọn 'Nâng cấp'";
            case DAP_SET_KICH_HOAT_TS_VIP:
                return "Cần 1 Trang Bị Thiên sứ bất kỳ và 999 viên ngọc 1 sao"
                        + "Sau đó chọn 'Nâng cấp'";
            case TAY_SKH_TRANG_BI:
            ...
            case AN_TRANG_BI:
                return "Vào hành trang\nChọn 1 Trang bị THIÊN SỨ và 99 nro\nSau đó chọn 'Làm phép'\n--------\n1 sao Tinh ấn (5 món +30%HP)\n2sao Nhật ấn (5 món +30%KI\n3sao Nguyệt ấn (5 món +20%SD)";
            case NANG_HUY_DIET_LEN_SKH_VIP:
                return "Vào hành trang\n Chọn 3 món huỷ diệt bất kỳ, món đầu tiên sẽ làm gốc, sau đó chọn 'Nâng cấp'";
            case AN_TRANG_BI_VIP:
                return "Vào hành trang\n Chọn 1 món thiên sứ kích hoạt,và x99 đá nâng cấp vip sau đó chọn 'Nâng cấp'";
            case SAO_9:
                return "Vào hành trang\n Chọn 1 món 8 sao pha lê,và x10 đá hematite + 1 dùi đục sau đó chọn 'Nâng cấp'";
            case NANG_CAP_CHAN_MENH:
                return "vào hành trang\nChọn Chân Mệnh\nVà 99 Đá Ngũ Sắc "
                        + "Bấm Nâng Cấp";
            case EP_CHUNG_NHAN_XUAT_SU:
